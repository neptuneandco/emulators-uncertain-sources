...
class SimpleBlock3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width, in_channels, out_channels, UNet=False):
...
class UFNO3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width, in_channels=6, out_channels=2, UNet=True):
        ...
        self.conv1 = SimpleBlock3d(modes1, modes2, modes3, width, in_channels, out_channels, UNet)
    ...
    def forward(self, x):
        ...
        # Calculate padding needed to make dimensions divisible by 8 for the U-Net
        pad_x = (8 - size_x % 8) % 8
        pad_y = (8 - size_y % 8) % 8
        pad_z = (8 - size_z % 8) % 8

        # Apply padding to the spatial dimensions (x, y, z).
        # The input tensor shape is (B, x, y, z, C).
        # F.pad pads from the last dimension backwards.
        padding_dims = (0, 0,      # No padding for channels (dim -1)
                        0, pad_z,   # Pad for z dimension (dim -2)
                        0, pad_y,   # Pad for y dimension (dim -3)
                        0, pad_x)   # Pad for x dimension (dim -4)

        x = F.pad(x, padding_dims, "constant")

        # Pass the padded tensor through the main block
        x = self.conv1(x)

        # Crop the output back to the original spatial dimensions
